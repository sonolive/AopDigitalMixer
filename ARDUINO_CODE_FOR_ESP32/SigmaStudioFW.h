...
void SIGMA_WRITE_REGISTER_BLOCK(int devAddress, int address, int length, ADI_REG_TYPE *pData )
{
	...
	txbuffer[0] =   (address & 0xFF00)>>8;   //mask the upper 8 bits of the address then put into unique variable
	txbuffer[1] =   address & 0x00FF;        //mask the lower 8 bits of the address then put into unique variable
	
	//fil the buffer array with the data (still leaving room for the address)

	for(zz=0;zz<length;zz++)
	{
		txbuffer[zz + Address_Length] = pData[zz];    //start fill buffer location after address
	}
	

	Wire.beginTransmission( devAddress ); // begin transmsion on dsp adress devAddress

	Wire.write( txbuffer , length ); // write length bytes

	Wire.endTransmission(); // end transmission = true
